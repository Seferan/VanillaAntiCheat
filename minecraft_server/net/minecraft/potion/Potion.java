...
public class Potion
{
    ...
    public static final Potion wither = (new Potion(20, true, 3484199))
            ...
            .setEffectiveness(0.25D);
    public static final Potion field_76434_w = (new PotionHealthBoost(21,
            false, 16284963))
    ...
    public void performEffect(EntityLivingBase par1EntityLivingBase, int par2)
    {
        ...
        else if (this.id == hunger.id
                && par1EntityLivingBase instanceof EntityPlayer)
        {
            ((EntityPlayer)par1EntityLivingBase)
                    .addExhaustion(0.025F * (float)(par2 + 1));
        }
        else if (this.id == field_76443_y.id
                && par1EntityLivingBase instanceof EntityPlayer)
        {
            if (!par1EntityLivingBase.worldObj.isClient)
            {
                ((EntityPlayer)par1EntityLivingBase).getFoodStats().addStats(
                        par2 + 1, 1.0F);
            }
        }
        else if ((this.id != heal.id || par1EntityLivingBase.isEntityUndead())
                && (this.id != harm.id || !par1EntityLivingBase
                        .isEntityUndead()))
        {
            if (this.id == harm.id && !par1EntityLivingBase.isEntityUndead()
                    || this.id == heal.id
                    && par1EntityLivingBase.isEntityUndead())
            {
                par1EntityLivingBase.attackEntityFrom(DamageSource.magic,
                        (float)(6 << par2));
            }
        }
        else
            ...
            EntityLivingBase par2EntityLivingBase, int par3, double par4)
    {
        ...
        if ((this.id != heal.id || par2EntityLivingBase.isEntityUndead())
                && (this.id != harm.id || !par2EntityLivingBase
                        .isEntityUndead()))
        {
            if (this.id == harm.id && !par2EntityLivingBase.isEntityUndead()
                    || this.id == heal.id
                    && par2EntityLivingBase.isEntityUndead())
            {
                var6 = (int)(par4 * (double)(6 << par3) + 0.5D);

                if (par1EntityLivingBase == null)
                {
                    par2EntityLivingBase.attackEntityFrom(DamageSource.magic,
                            (float)var6);
                }
                else
                {
                    par2EntityLivingBase.attackEntityFrom(DamageSource
                            .causeIndirectMagicDamage(par2EntityLivingBase,
                                    par1EntityLivingBase), (float)var6);
                }
            }
        }
        else
    ...
    public Potion func_111184_a(IAttribute par1Attribute, String par2Str,
            double par3, int par5)
    {
        AttributeModifier var6 = new AttributeModifier(
                UUID.fromString(par2Str), this.getName(), par3, par5);
            ...
            BaseAttributeMap par2BaseAttributeMap, int par3)
    {
        ...
        while (var4.hasNext())
        {
            ...
            IAttributeInstance var6 = par2BaseAttributeMap
            ...
            BaseAttributeMap par2BaseAttributeMap, int par3)
    {
        ...
        while (var4.hasNext())
        {
            ...
            if (var6 != null)
            {
                ...
                var6.applyModifier(new AttributeModifier(var7.getID(), this
                        .getName() + " " + par3,
                        this.func_111183_a(par3, var7), var7.getOperation()));
            }
        }
    }

    public double func_111183_a(int par1,
